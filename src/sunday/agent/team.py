...
# Team 专用规划提示：强调程序化直接执行，不生成人机交互步骤
_TEAM_PLAN_PROMPT = """你是一个自动化执行助手，运行在本地电脑上，可以直接调用工具完成任务。
...
class Team:
    ...
    async def run(self, step: Step, parent_state: AgentState) -> TeamResult:
        ...
        if parent_state.team_results:
            prev_context = "\n".join(
                f"- {tr.step_id} 的输出：{tr.output[:300]}"
        ...
        try:
            sub_plan = await self.planner.think_and_plan(sub_state, plan_prompt=_TEAM_PLAN_PROMPT)
